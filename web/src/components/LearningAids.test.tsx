...
const jefe = {
  ...MEMBER,
  progress: { ...MEMBER.progress, level: 5, aids: { formulas: false, learn_more: false, hint: true } },
}
const dt = {
  ...jefe,
  progress: { ...jefe.progress, level: 10, aids: { formulas: false, learn_more: false, hint: false } },
}
...
test('a hint rules out options, says what it costs and is asked for once', async () => {
  ...
  await userEvent.click(await screen.findByRole('button', { name: 'Hint (halves the XP)' }))
  expect(
    await screen.findByText(/Two options left: one of them is right\. Right answers now earn half the XP\./),
  ).toBeInTheDocument()
  ...
  expect(screen.queryByRole('button', { name: 'Hint (halves the XP)' })).toBeNull()
