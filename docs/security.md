...
- **Health endpoints:** `/healthz` and `/readyz` need no sign-in and return only a status (`/healthz` also the version). `/readyz` runs one `SELECT 1` from the app's connection pool per call; the cause of a failure goes to the api log, never the response.
