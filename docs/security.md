...
| Stolen session | Random 256-bit session ID in a `__Host-` cookie (HttpOnly, Secure, SameSite=Lax); staging and prod refuse to start unless served over https, and only local http development uses a plain `sid` cookie, because some browsers drop Secure cookies on http://localhost; only its hash is stored; 12 h idle / 30 d absolute; rotated at login; ended by password change/reset or disabling; admins can revoke |
