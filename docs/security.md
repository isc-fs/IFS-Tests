...
| Leaked invite or reset links | 256-bit single-use tokens, hashed at rest; carried in the URL fragment and sent in POST bodies, so they never reach access logs; a password change or reset closes every open reset link |
...
| Look-alike names | Display names are NFKC-normalised and limited to Latin letters, digits and `. ' -` |
