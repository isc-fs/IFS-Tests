...
- **Review** (`services/review.py`, reviewers and admins): queues for reported questions, questions FS-Quiz changed, unclassified, not graded and hidden ones. Reviewers fix area and topic (kept on re-import), hide a question from every mode, or correct its answer (a reviewer's key used instead of FS-Quiz's; a typed correction can make a reveal-only question gradable). If FS-Quiz changes a question's content, a correction is dropped and the question goes back to the queue; if only its images arrive, options and corrections stay. Players can report a problem after answering. Every reviewer action is audited.
...
- **Leaderboard** (`domain/leaderboard.py`, `services/leaderboard.py`): points from daily questions and mock quizzes (practice scores nothing), summed per active member over this season (from 1 September, Madrid time) or the last 7 Madrid days, today included. Boards: everyone, and one per area (a daily question counts for its area, a mock answer for its question's area). Competition ranking with ties (1, 2, 2, 4), top 50 shown; you only appear once you have scored in the period. Alumni and disabled accounts never appear. People who opted out are left out of the rows and the ranking, but always get their own rank and points as if they were included, as does anyone outside the top 50. The vertical board shows, for each vertical with at least 3 active members, the average points per active member (everyone counted, opt-outs included but never named) and participation: the share of its members with a submitted daily answer in the last 7 Madrid days. Members without a vertical count for none.
