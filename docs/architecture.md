...
- **XP and levels** (`domain/xp.py`, `services/xp.py`, [ADR 0004](adr/0004-xp-and-levels.md)): one currency. Each answer earns base XP by the question's difficulty (1–5) × mode (practice ½, daily 2, mock 1½) × streak bonus (+5 % a day, up to +50 %). Lifetime XP sets the level (`25 × L^2.3`) and its title; levels take the training wheels away (formulas, reading, hints) and make wrong answers cost a growing share of what a right one earns (nothing up to level 7, 75 % from level 20). Ranks chosen at sign-up (Mingo, returning member, Department Head, Technical Director) set the starting level, and lifetime XP never drops below it. Difficulty starts from the answer type and time budget and is recalibrated nightly from success rates once 20 people have answered. Seasons run September–August.
