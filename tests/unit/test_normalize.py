from ifs_tests.bank.normalize import build_bank, clean_text, parse_range, question


def test_clean_text_fixes_escaped_newlines() -> None:
    assert clean_text("a\\nb\r\nc\xa0d ") == "a\nb\nc d"
...
def test_question_accepts_live_quirks() -> None:
    raw = {
        "question_id": "90",
        "type": "input-range",
        "text": "x\\ny",
        ...
        "solution": [
            {
                ...
                "question_id": 90,
                "text": None,
                "images": [{"img_id": 1, "path": "solutions/90_1.jpg"}],
            }
        ],
    }
    q = question(raw)
...
def test_build_bank_dedupes_shared_questions() -> None:
    q = {
        "question_id": 5,
        "type": "single-choice",
        "text": "t",
    ...
    quizzes = [
        {
            ...
            "class": "ev",
            "status": "complete",
            "event": [{"event_id": 7, "short_name": "FSG"}],
            "questions": [q | {"position_index": 3}],
            "documents": [],
        },
        {
            "quiz_id": 2,
            ...
            "class": "ev",
            "status": "complete",
            "event": [{"event_id": 1, "short_name": "FSN"}],
            "questions": [q | {"position_index": 1}],
            "documents": [],
        },
    ]
