...
def export(db: DB, user: User, now: datetime) -> dict[str, Any]:
    ...
    mock_runs = [
        {"quiz_id": m.quiz_id, "season": m.season, "started_at": m.started_at, "finished_at": m.finished_at}
        for m in db.scalars(
            select(MockSession).where(MockSession.user_id == user.id).order_by(MockSession.id)
        )
    ]
    sessions = db.execute(
        select(LiveSession.id, LiveSession.code, LiveSession.created_at, LivePlayer.joined_at, LiveTable.name)
    ...
    hosted = db.scalars(
        select(LiveSession.code).where(LiveSession.host_id == user.id).order_by(LiveSession.id)
    )
    sent = [
        {
            "code": code,
    ...
    return {
        ...
        "live": {
            "joined": [
                {"code": code, "created_at": created, "joined_at": joined, "table": table}
                for _, code, created, joined, table in sessions
            ],
            "hosted": list(hosted),
        ...
        # What happened to the account; what they did to others' accounts stays out (it's about them).
        "account_history": [
...
def _lock(db: DB, user_id: int) -> tuple[list[int], User | None]:
    """Admin rows first, in id order like every admin change, then the account: two admins leaving at
    once queue up instead of deadlocking, and the second sees it would leave no admin."""
    admins = _active_admin_ids(db)
    return admins, db.get(User, user_id, with_for_update=True, populate_existing=True)
...
def _delete(db: DB, user: User, now: datetime) -> None:
    ...
    live.finish_hosted(db, user.id, now)
    db.execute(
        update(LiveAnswer)
        .where(LiveAnswer.member_ids.contains([user.id]))
        .values(member_ids=func.array_remove(LiveAnswer.member_ids, user.id))
    )
    # Their invite named them: drop the note there and in the invite's audit entry.
    invites = [f"invite:{i}" for i in db.scalars(select(Invite.id).where(Invite.used_by == user.id))]
    db.execute(update(Invite).where(Invite.used_by == user.id).values(note=None))
    if invites:
        db.execute(
            update(AuditLog)
            .where(AuditLog.action == "invite.create", AuditLog.target.in_(invites))
            .values(details=AuditLog.details.op("-")("note"))
        )
    db.delete(user)
...
def delete_user(db: DB, actor: User, user_id: int, now: datetime) -> None:
    ...
    admins, user = _lock(db, user_id)
    if user is None:
        raise AccountError("No such user.", 404)
    _guard_last_admin(admins, user)
    audit(db, actor, "user.delete", f"user:{user.id}", by="admin")
    _delete(db, user, now)
    db.commit()
...
def mark_alumni(db: DB, actor: User, user_ids: list[int], now: datetime) -> int:
    ...
    ids = set(user_ids) - {actor.id}  # the acting admin stays, so an active admin always remains
...
def purge(db: DB, now: datetime) -> dict[str, int]:
    """Nightly: alumni a year after they left, and audit entries past their keep."""
    due = db.scalars(
        select(User).where(User.status == "alumni", User.left_at <= now - rules.ALUMNI_KEEP).with_for_update()
    ).all()
    for u in due:
        audit(db, None, "user.delete", f"user:{u.id}", by="retention")
        _delete(db, u, now)
    old = rowcount(db.execute(delete(AuditLog).where(AuditLog.at < now - rules.AUDIT_KEEP)))
    return {"alumni_deleted": len(due), "audit_purged": old}
