...
def salt(db: DB) -> bytes:
    """A server secret for anything drawn per question that players mustn't be able to recompute."""
    db.execute(
        insert(Setting)
        .values(key="hint_salt", value={"salt": secrets.token_hex(32)})
        .on_conflict_do_nothing()
    )
    return bytes.fromhex(db.get_one(Setting, "hint_salt").value["salt"])
...
def _seed(db: DB, question_id: int) -> int:
    ...
    digest = hmac.new(salt(db), str(question_id).encode(), hashlib.sha256).digest()
    return int.from_bytes(digest[:8], "big")


def _hint(db: DB, user: User, q: Question, now: datetime) -> rules.Hint:
    if not rank_rules.standing(user.rank_points, user.rank_season, user.position, None, now).division.hint:
...
def practice(db: DB, user: User, question_id: int, now: datetime) -> rules.Hint:
    ...
    h = _hint(db, user, playable(db, question_id), now)
    db.execute(
        insert(PracticeHint)
...
def timed(db: DB, user: User, attempt_id: int, now: datetime, session_id: int | None = None) -> rules.Hint:
    ...
    h = _hint(db, user, db.get_one(Question, a.question_id), now)
