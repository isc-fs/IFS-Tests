...
def ranks(points: Sequence[int]) -> list[int]:
    """Competition ranking of points sorted high to low: 30, 20, 20, 10 -> 1, 2, 2, 4."""
    out: list[int] = []
    for i, p in enumerate(points):
        out.append(out[-1] if i and p == points[i - 1] else i + 1)
    return out


def rank_among(points: int, others: Iterable[int]) -> int:
    """Where someone with `points` would stand among `others`, sharing the rank of anyone level."""
    return 1 + sum(1 for p in others if p > points)
...
def vertical_board(members: Iterable[Member]) -> list[VerticalScore]:
    ...
    rows = [
        VerticalScore(
            vertical=v,
            members=len(ms),
            points_per_member=round(sum(m.points for m in ms) / len(ms), 1),
            participation=round(sum(m.played_this_week for m in ms) / len(ms), 3),
        )
        for v, ms in by_vertical.items()
        if len(ms) >= MIN_VERTICAL
    ]
    return sorted(rows, key=lambda r: (-r.points_per_member, -r.participation, r.vertical))
